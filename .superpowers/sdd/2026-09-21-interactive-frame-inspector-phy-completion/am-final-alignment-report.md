...
The fixture now references `marker-final-cl119-derivation-v1.json`, the
provenance record for the standalone Python reference. It records the
final-source filename, SHA-256, edition, pages, table and figure, deterministic
JSON canonicalization, script revision and hash, and independent review scope.
The focused test checks the frozen marker-bit SHA-256, script SHA-256, and the
artifact's canonical SHA-256 before using the fixture as an expected value.

## Independent reference correction

The expected marker is now generated by the standalone
`scripts/marker-reference.py`, which imports no production marker code or
fixture data. Independent review `/root/marker_reference_review_light` found
and corrected Table 119-2 lane 5 byte 13 to `0xED` and lane 12 byte 9 to
`0xA2`. The corrected product-policy marker SHA-256 is
`d2d54fc33e0b658060e0a146ccd0bbf08b19250cb5a832b8a7a94f711ef976bd`.
The manifest records the script revision and SHA-256, source PDF hash and
locators, input state, reviewer identity, review date and reviewed scope.
Artifact canonicalization sorts object keys by Unicode code point, not locale,
and the TypeScript check uses the same ordering. The PRBS orientation and seed,
and status `000`, remain explicitly product choices rather than claimed
normative wire values.
