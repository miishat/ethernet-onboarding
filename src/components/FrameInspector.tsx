...
import type { FrameDraft, FrameInput, InspectorStage, MacFrame } from "../inspector/types";
...
interface Props { stage: InspectorStage; onStageChange: (stage: InspectorStage) => void; onExit: () => void; draft: FrameDraft; onDraftChange: (draft: FrameDraft) => void; mac: MacFrame | null; onApply: (input: FrameInput) => void; }

function hex(bytes: Uint8Array) { return Array.from(bytes, (byte) => byte.toString(16).padStart(2, "0")).join(" "); }

export default function FrameInspector({ stage, onStageChange, onExit, draft, onDraftChange, mac, onApply }: Props) {
  ...
  return <main className="inspector" aria-label="Frame inspector">
    ...
    {unavailable ? <section className="inspector__unavailable" aria-label="Unavailable calculation" aria-live="polite"><h2>{stage.replace(/-/g, " ")}</h2><p>Calculation not available yet. This stage depends on a PHY profile that is verification-blocked, so no simulated values are displayed.</p></section> : <>
