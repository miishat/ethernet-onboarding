...
export default function FrameInspector({ stage, onStageChange, onExit, draft, onDraftChange, prefixIdleOctets = 256, onPrefixIdleOctetsChange, mac = null, onApply, run = null, status = "idle", error = null, calculationUnavailableReason = null }: Props) {
  ...
  return <main className="inspector" aria-label="Frame inspector">
    <div className="inspector__top"><div><p className="inspector-eyebrow">Experimental candidate calculation</p><h1>MAC frame inspector</h1><p>Applied values use a candidate reference mapping; IEEE-only 400G TX remains verification-blocked.</p></div><button className="btn" type="button" onClick={onExit}>Return to learning</button></div>
