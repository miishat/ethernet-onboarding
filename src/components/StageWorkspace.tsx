...
export default function StageWorkspace({ run, stage, selected, onSelect }: { run: CompleteInspectorRun; stage: InspectorStage; selected?: DataRef | null; onSelect?: (ref: DataRef) => void }) {
  ...
  return <section className="stage-workspace" aria-label={`${stage === "pcs-lanes" ? "PCS lanes" : stage.replace(/-/g, " ")} workspace`}><div className="inspector-section-head"><div><p className="inspector-eyebrow">{reference ? "reference PMA mapping" : "IEEE-derived PCS candidate"}</p><h2>{stage.replace(/-/g, " ")}</h2></div><span>{snapshot.explanation}</span></div><p className="stage-workspace__applied-input">Applied worker result FCS: <code>{fcs}</code></p><div className="provenance">{run.provenance.labels.map((label) => <span key={label}>{label}</span>)} {snapshot.referenceIds.map((id) => <code key={id}>{id}</code>)}</div>{selected ? <p className="trace-details" aria-live="polite">Selected {selected.stage} {selected.bufferId} at absolute index {selected.start}. {connected.length ? `Trace: ${connected.map((edge) => `${edge.relation} (${edge.precision})`).join(", ")}.` : "No connected trace edge."}</p> : null}{content}</section>;
}
