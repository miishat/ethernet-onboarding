...
export default function StageWorkspace({ run, stage }: { run: CompleteInspectorRun; stage: InspectorStage }) {
  ...
  const content = stage === "pcs-lanes" ? <LaneView run={run} /> : stage === "physical-lanes" ? <LaneView run={run} physical /> : stage === "pam4" ? <Pam4View run={run} /> : <DataWindow snapshot={snapshot} />;
