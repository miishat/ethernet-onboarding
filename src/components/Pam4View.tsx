...
export default function Pam4View({ run, view = "indexed", selected, linked = [], onSelect }: { run: CompleteInspectorRun; view?: ValueView; selected?: DataRef | null; linked?: readonly DataRef[]; onSelect?: (ref: DataRef) => void }) {
  ...
  return <section className="pam4-view" aria-label="PAM4 values">
    ...
    <label className="inspector-select">PMD lane <span className="inspector-select__control"><select aria-label="PMD lane" value={lane} onChange={(event) => { setLane(Number(event.target.value)); setStart(0); }}>{[0, 1, 2, 3].map((value) => <option key={value} value={value}>PMD lane {value}</option>)}</select></span></label>
