...
export default function LaneView({ run, physical = false, view = "indexed", selected, linked = [], onSelect }: { run: CompleteInspectorRun; physical?: boolean; view?: ValueView; selected?: DataRef | null; linked?: readonly DataRef[]; onSelect?: (ref: DataRef) => void }) {
  ...
  useEffect(() => {
    if (!focus) return;
    const laneWidth = physical ? laneBitLength : rows.length * 10;
    const focusedLane = Math.floor(focus.start / laneWidth);
    const item = physical ? focus.start % laneWidth : Math.floor((focus.start % laneWidth) / 10);
    setLane(focusedLane);
  ...
  return <section className="lane-view" aria-label={physical ? "Physical lane values" : "PCS lane values"}>
    ...
    <label className="inspector-select">Lane <span className="inspector-select__control"><select aria-label="Lane" value={lane} onChange={(event) => { setLane(Number(event.target.value)); setStart(0); }}>{Array.from({ length: physical ? 4 : 16 }, (_, value) => <option key={value} value={value}>{physical ? "PMD" : "PCS"} lane {value}</option>)}</select></span></label>
