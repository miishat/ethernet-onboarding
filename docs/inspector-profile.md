...
The proposed phase convention is: marker phase zero names the boundary immediately before the first bit of an inserted alignment-marker group; lane phase zero names the first distribution unit assigned to logical lane zero at that same boundary. The research identifies a candidate 40-block, two-codeword boundary, but a final IEEE line check is still required before phase fields drive computation. A `RateMatchPolicy` is also required: IEEE does not prescribe where an implementation removes individual eligible Idles to make AM capacity. That policy must declare eligible Idle encodings, selection strategy, maximum deferral, frame-boundary behaviour, phase relationship and a deletion trace. In particular, zero phase does not imply that 4096 prefix idles can simply be added after a marker.
...
| `rate-match-policy` | **Implementation-specific.** IEEE Working Group material says it does not prescribe individual Idle deletion locations. | `RateMatchPolicy` must define eligible Idles, selection, maximum deferral, frame boundaries, AM phase and complete deletion trace. |
