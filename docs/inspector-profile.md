...
`PmaMappingProfile` describes one observable PMA/module implementation. It must include provenance and fixture hash; exact scope/boundary; 16:4 geometry; each PMD lane's ordered bit-level PCS schedule and phase; boundary bit indexing; PMD-to-MDI and MDI-to-fiber maps; polarity transform and stage; dibit order/significance; full dibit-to-label-and-normalized-level map; precoder mode/state/reset; and the PMA training/PCS-scrambler boundary. An illustrative mapping must carry an illustrative profile ID and cannot enable this profile.
