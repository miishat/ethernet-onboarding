...
- AM Idle deletion is implementation-owned. Before a finite default stream can exist, select deterministic `RateMatchPolicy` with eligible Idles, ordered selection/tie-break and deferral rules, frame-boundary behaviour, a verification-gated AM cadence in transcoded 257-bit blocks, exact phase zero, a next-reservation function from absolute stream phase, and a reproducible deletion ledger.
...
- [ ] Pin one deterministic initialization only after the final line check accepts the recurrence. The requested 58 one-bit teaching seed, PRBS seed `0x1ff`, marker/lane phase zero and 4,096 prefix Idle count remain teaching metadata, not hardware reset state. Add a deterministic, serializable `RateMatchPolicy`: eligible Idle controls; ordered candidate rule and tie-breaker; maximum deferral; frame-boundary rule; and an AM schedule in transcoded 257-bit blocks. Its phase zero is absolute stream block 0 immediately before a marker reservation, and its verification-gated cadence and reservation capacity calculate `nextReservation(b) = phaseZero + max(0, ceil((b - phaseZero) / cadenceBlocks)) * cadenceBlocks` from carried absolute block `b`. Derive the deletion ledger from that policy plus the original input words and state, never from a prior render. Its product-owned policy determines trailing Idles to complete codeword and lane groups. Cap the inspected result at 262,144 coded bits; reject over-cap requests instead of truncating a frame. Exercise later marker boundaries using phase-controlled windows with carried state and deletion trace rather than storing a full marker period.
...
Add `RateMatchPolicy={id:string; eligibleIdleEncodings:readonly IdleEncoding[]; selection:{order:'nearest-before-reservation'; tieBreak:'lowest-original-bit-offset'}; maximumDeferralBlocks:number; frameBoundaryRule:string; amSchedule:{unit:'transcoded-257b-block'; phaseZeroAbsoluteStreamBlock:0; cadenceBlocks:number; reservationBlocks:number; fecPairBlocks:number; nextReservation:'phaseZero + max(0, ceil((absoluteStreamBlock - phaseZero) / cadenceBlocks)) * cadenceBlocks'}}` and `RateMatchDeletion={originalPosition:{wordIndex:number;octetIndex:number;bitOffset:number}; encoding:IdleEncoding; policyId:string; reason:'alignment-marker-reservation'|'trailing-fec-completion'; markerReservation:{groupIndex:number;insertionBitOffset:number;fecPairIndex:number;boundary:'before-fec-pair'|'trailing-completion'}}`, where `IdleEncoding={octets:Uint8Array;controlMask:number}`. `cadenceBlocks` and `reservationBlocks` are verification-gated until the final IEEE check, and `nextReservation` operates on the carried absolute pre-insertion transcoded-block position. Add `MarkerPlan={insertionBitOffsets:readonly number[]; rateMatchLedger:readonly RateMatchDeletion[]; finalPhase:number}`, `PreparedStream={words:readonly InterfaceWord[]; markerPlan:MarkerPlan; rateMatchLedger:readonly RateMatchDeletion[]}`, and `MarkerResult={bits:Uint8Array; markers:readonly {bitOffset:number;bitLength:number}[]; rateMatchLedger:readonly RateMatchDeletion[]; finalPhase:number}`. The planner must derive the ledger solely from the policy, initial AM state and original input words, applying the declared candidate order and tie-break at each `nextReservation`; it must not reuse a ledger from a prior displayed window. The ledger, not a `removedIdleOctets` aggregate, is the authoritative record. An aggregate count may be derived only for display. Offsets in `MarkerPlan` are positions in the scrambled stream before insertion, in ascending order. Marker insertion consumes already scrambled bits and a precomputed plan; it cannot infer deletable Idle locations from scrambled bits.

- [ ] Freeze scrambler vectors including zero input, varied input, and chunk boundaries; marker vectors cover before/at/after insertion, final phase, pad sequence, status bits, and every `RateMatchDeletion` field. For the same policy, initial AM state and original input, regenerate the ledger from a later window and require byte-for-byte equality with the full-run ledger. Include more than one eligible Idle and assert the selected original position/encoding, policy/reason and marker-reservation boundary, not only a deletion count. Write:
